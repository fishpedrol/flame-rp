...
oC = {}
Tunnel.bindInterface("fsh_crafting",oC)
-----------------------------------------------------------------------------------------------------------------------------------------
--[ ARRAY ]------------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
local craft = {
                    ...
                    if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("wammo_WEAPON_PISTOL_MK2") <= vRP.getInventoryMaxWeight(user_id) then
                        if vRP.getInventoryItemAmount(user_id,"distintivopolicial") >= 6 then
                            if vRP.tryGetInventoryItem(user_id,"distintivopolicial",6) then
                                ...
                                SetTimeout(5000,function()
                                    vRPclient._stopAnim(source,false)
                                    vRP.giveInventoryItem(user_id,"wammo_WEAPON_PISTOL_MK2",25)
                                    TriggerClientEvent("Notify",source,"sucesso","Você montou uma <b>M.Five Seven</b>.")
                                end)
                            end
                        else
