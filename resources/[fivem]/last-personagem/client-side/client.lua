RegisterNetEvent("last-personagem:startIntro")
RegisterNetEvent("last-personagem:normalSpawn")
RegisterNetEvent("last-personagem:characterCreate")
...
function drawscaleform(scaleform)
	...
	PushScaleformMovieFunctionParameterString("~r~Last Ultimate")
	PushScaleformMovieFunctionParameterBool(true)
	PushScaleformMovieFunctionParameterString("~y~discord.gg/lastultimate")
...
-- EVENTS --
AddEventHandler("last-personagem:startIntro", function()
	print("last-personagem:startIntro")
	doStatus = -1
	SetTimeout(1000, function()
		TriggerServerEvent("last-personagem:finishedIntro")
	end)
end)



AddEventHandler("last-personagem:characterCreate", function()
	print("last-personagem:characterCreate")
	doStatus = 1
...
AddEventHandler("last-personagem:normalSpawn",function(firstspawn)
	if firstspawn then
...
function refreshDefaultCharacter()
	...
	if GetEntityModel(PlayerPedId()) == GetHashKey("mp_m_freemode_01") then
		...
		SetPedComponentVariation(PlayerPedId(),11,74,0,2)
		SetPedComponentVariation(PlayerPedId(),9,-1,0,2)
		SetPedComponentVariation(PlayerPedId(),10,-1,0,2)
		SetPedPropIndex(PlayerPedId(),2,-1,0,2)
		SetPedPropIndex(PlayerPedId(),6,-1,0,2)
...
function changeGender(model)
	...
	if HasModelLoaded(mhash) then
		...
		SetPedMaxHealth(PlayerPedId(),700)
		SetEntityHealth(PlayerPedId(),700)
...
RegisterNUICallback('cDoneSave', function(data, cb)
	...
	TriggerServerEvent("last-personagem:finishedCharacter", characterNome, characterSobrenome, characterAge, currentCharacterMode)
end)
