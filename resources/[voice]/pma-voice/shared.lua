...
if GetConvar('voice_useNativeAudio', 'false') == 'true' then
	...
	Cfg.voiceModes = {
		{1.5, "Sussurrando"}, -- Whisper speech distance in gta distance units
		{3.0, "Normal"}, -- Normal speech distance in gta distance units
		{6.0, "Gritando"} -- Shout speech distance in gta distance units
	}
else
	Cfg.voiceModes = {
		{3.0, "1"}, -- Whisper speech distance in gta distance units
		{7.0, "2"}, -- Normal speech distance in gta distance units
		{15.0, "3"} -- Shout speech distance in gta distance units
	}
end
