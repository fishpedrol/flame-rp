...
var updateMochila = () => {
  ...
  $.post("http://vrp_inventory/requestMochila", JSON.stringify({}), data => {
    ...
    Inventário <small>Utilizando <b style="color: rgb(255, 255, 255);">${data.peso.toFixed(2)} KG</b> • Livre <b style="color: rgb(255, 255, 255);">${(data.maxpeso - data.peso).toFixed(2)} KG</b> <b style="color: rgba(36,123,227,255);>${data.maxpeso.toFixed(2)}kg</b> no total)</small>
    `);
    $(".row.objects").html(`
    ${nameList
        .map(
        ...
        <div class="options">
          ...
          <div class="option" data-event="drop">Destruir</div>
        </div>
        ...
        <div class="row">
        <div class="image" style="background-image: url(images/${item.index}.png)"></div>
        </div>
        <div class="row">
