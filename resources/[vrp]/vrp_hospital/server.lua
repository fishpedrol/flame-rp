	...
	if vRP.hasPermission(user_id,"paramedico.permissao") then
		...
		if nplayer then
			if vRPclient.isInComa(nplayer) then
				...
				TriggerClientEvent("Notify",source,"importante","Sistema","A pessoa precisa estar em coma para prosseguir.")
			end
		end
	end
end)
...
function Resg.checkPayment()
	...
	if user_id then
		if vRP.tryFullPayment(user_id,4000) then
	 		TriggerClientEvent("Notify",source,"sucesso","Pagamento de <b>$4.000 dólares</b> efetuado e o tratamento foi iniciado.")
	 		return true
		else
...
function Resg.receiveBandagem()
	...
	if user_id then
		if vRP.hasPermission(user_id,"paramedico.permissao") then
			if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("bandagem")*5 <= vRP.getInventoryMaxWeight(user_id) then
				if vRP.tryPayment(user_id,2000) then
					vRPclient._playAnim(source,false,{{"amb@prop_human_parking_meter@female@idle_a","idle_a_female"}},true)
					TriggerClientEvent("cancelando",source,true)
					TriggerClientEvent("progress",source,5000,"PRODUZINDO")
					TriggerClientEvent("Notify",source,'importante',"Sistema","Você gastou <b>$2.000 dólares</b> para fazer bandagem")
					...
					TriggerClientEvent("Notify",source,"negado","Você precisa de <b>$2.000 dólares</b> para finalizar a produção.",8000)
				end
			else
...
function Resg.checkParamedic()
	local medical = vRP.getUsersByPermission("paramedico.permissao")
	if #medical <= 0 then
		return false
	else 
		return true
...
function Resg.receiveBandagemOutMedic()
	local source = source
	local user_id = vRP.getUserId(source)
	TriggerClientEvent("progress",source,2000,"COMPRANDO")
	vRPclient._playAnim(source,false,{{"amb@prop_human_parking_meter@female@idle_a","idle_a_female"}},true)
	vRP.tryFullPayment(user_id,7000)
