...
AddEventHandler('tratamento-macas',function()
	...
	TriggerEvent("Notify","Sistema","Tratamento concluido.")
...
Citizen.CreateThread(function()
	while true do
		...
		local x,y,z = table.unpack(GetEntityCoords(ped))
		local distancea = Vdist(x,y,z,checkinX,checkinY,checkinZ)
		if distancea <= 2.0 then
...
Citizen.CreateThread(function()
	while true do 
		local ped = GetPlayerPed(-1)
		local x,y,z = table.unpack(GetEntityCoords(ped))
		local distance = Vdist(x,y,z,308.01,-595.02,43.3)
	   	if distance <= 2.0 then
			text3D(308.01,-595.02,43.3,"~g~E~w~  PARA COMPRAR BANDAGEM [$7.000]")
			if IsControlJustPressed(1,38) then
				Resg.receiveBandagemOutMedic()
				TriggerEvent("Notify","sucesso","Voce comprou <b>1<b> bandagem.",8000)
			end
		end
		Citizen.Wait(1)
	end
