...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookjoinip = "https://discord.com/api/webhooks/1214679949601013852/sSpGkJF9JsGuj7ENgJCj0K67i04fTTgxjb53WzSXeA0jdHHzLeI-gsP2rHob00-b0VFu" -- teste
local webhookexitip = "https://discord.com/api/webhooks/1214680015560638464/ZeOli7jY7V2193FMVhfspAgpduyKnDfbwcqXM5P3JuIkQ25WW-2yHjYCIvGB28Fyw7uM" -- teste 
...
function vRP.dropPlayer(source)
	...
	if user_id then
		if user_id and source then
			...
			SendWebhookMessage(webhookexitip,"```prolog\n[ID]: "..user_id.." \n[=========SAIU DO SERVIDOR=========]"..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
			...
			if not vRP.isBanned(user_id) then
				...
				if vRP.isWhitelisted(user_id) then
					if vRP.rusers[user_id] == nil then
						...
						SendWebhookMessage(webhookjoinip,"```prolog\n[ID]: "..user_id.." \n[ENTROU NO SERVIDOR]: "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
						deferrals.done()
					else
						local tmpdata = vRP.getUserTmpTable(user_id)
						tmpdata.spawns = 0
					...
					deferrals.done("Solicite sua whitelist em nosso Discord! https://discord.gg/falldevs ID: "..user_id.." ")
					TriggerEvent("queue:playerConnectingRemoveQueues",ids)
				end
			else
