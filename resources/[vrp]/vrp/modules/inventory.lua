...
local itemlist = {
	...
	["caixadelsd"] = { index = "caixadelsd", nome = "Caixa de LSD", type = "usar" },
	...
	["armacaodearma"] = { index = "armacaodearma", nome = "Armação Simples", type = "usar" },
...
function vRP.giveInventoryItem(user_id,idname,amount)
    ...
    if data and amount > 0 then
        ...
        local getSource = vRP.getUserSource(user_id)
        TriggerClientEvent("itensNotify", getSource, "sucesso", idname, vRP.itemNameList(idname), amount)
    end
end
...
function vRP.tryGetInventoryItem(user_id,idname,amount)
    ...
    if data and amount > 0 then
        ...
        if entry and entry.amount >= amount then
            ...
            local getSource = vRP.getUserSource(user_id)
            TriggerClientEvent("itensNotify", getSource, "negado", idname, vRP.itemNameList(idname), amount)
            return true
        end
    end
    return false
...
RegisterServerEvent('bl:GuardarArmasMorte')
AddEventHandler('bl:GuardarArmasMorte', function()
   local source = source
   local user_id = vRP.getUserId(source)
   if user_id then
...
RegisterServerEvent('bl:Aeroporto:Server')
AddEventHandler('bl:Aeroporto:Server', function(lugar)
	...
	if not vRP.hasPermission(user_id,"mochila.permissao") and not vRP.hasPermission(user_id, 'policiacao.permissao') then
		vRP.setExp(user_id,"physical","strength",20)
	end
	
	-- ENVIA O LOG PRO DISCORD
	local x,y,z = vRPclient.getPosition(source)
	TriggerEvent('bl:LoggarAeroporto', source, x,y,z)

	-- RESPAWNA
	local pulica = false
	if vRP.hasPermission(user_id, 'policia.permissao') or vRP.hasPermission(user_id, 'policiacao.permissao') then
		pulica = true
	end

	--TriggerClientEvent('bl:Aeroporto:Client', source, lugar,pulica)
...
RegisterServerEvent('bl:nascerbase')
AddEventHandler('bl:nascerbase', function()
    TriggerClientEvent('bl:Aeroporto:Client2', source)
    local source = source
    local user_id = vRP.getUserId(source)
    if vRP.hasPermission(user_id, "policia.permissao") then
            vRPclient._teleport(source, 642.43,-0.11,82.79)
            TriggerClientEvent('bl:ZerarInv', source)

    elseif vRP.hasPermission(user_id, "policiacao.permissao") then
            vRPclient._teleport(source, 642.43,-0.11,82.79)
            TriggerClientEvent('bl:ZerarInv', source)

    elseif vRP.hasPermission(user_id, "paramedico.permissao") then
            vRPclient._teleport(source, 293.07,-603.88,43.32)
            TriggerClientEvent('bl:ZerarInv', source)

    elseif vRP.hasPermission(user_id, "paramedicopaisana.permissao") then
            vRPclient._teleport(source, 293.07,-603.88,43.32)
            TriggerClientEvent('bl:ZerarInv', source)

    elseif vRP.hasPermission(user_id, "vagos.permissao") then
            vRPclient._teleport(source, 342.36,-2043.25,21.46)
            TriggerClientEvent('bl:ZerarInv', source)

    elseif vRP.hasPermission(user_id, "ballas.permissao") then
            vRPclient._teleport(source, 114.01,-1948.63,20.61)
            TriggerClientEvent('bl:ZerarInv', source)

    elseif vRP.hasPermission(user_id, "families.permissao") then
            vRPclient._teleport(source, -165.66,-1666.02,33.09)
            TriggerClientEvent('bl:ZerarInv', source)

    elseif vRP.hasPermission(user_id, "bloods.permissao") then
            vRPclient._teleport(source, -1076.31,-1663.81,4.4)
            TriggerClientEvent('bl:ZerarInv', source)

    elseif vRP.hasPermission(user_id, "crips.permissao") then
            vRPclient._teleport(source, 1281.45,-1732.3,52.66)
            TriggerClientEvent('bl:ZerarInv', source)

    elseif vRP.hasPermission(user_id, "mafiarussa.permissao") then
            vRPclient._teleport(source, 589.82,-3120.99,6.07)
            TriggerClientEvent('bl:ZerarInv', source)

    elseif vRP.hasPermission(user_id, "mafiaitaliana.permissao") then
            vRPclient._teleport(source, -1523.6,855.18,181.6)
              TriggerClientEvent('bl:ZerarInv', source)
            vRPclient.SetEntityHealth(source, 398)
    else
            vRPclient._teleport(source, -347.32,-874.76,31.09)
        TriggerClientEvent('Notify', source, 'negado', 'Somente membros de fac/org podem ir para base então você foi teleportado para a garagem principal.')
        TriggerClientEvent('bl:ZerarInv', source)
...
local vehglobal = {
	...
	["issi2"] = { ['name'] = "Issi2", ['price'] = 60000, ['tipo'] = "carros" },
	["panto"] = { ['name'] = "Panto", ['price'] = 10000, ['tipo'] = "carros" },
	["prairie"] = { ['name'] = "Prairie", ['price'] = 30000, ['tipo'] = "carros" },
	["rhapsody"] = { ['name'] = "Rhapsody", ['price'] = 20000, ['tipo'] = "carros" },
	["cogcabrio"] = { ['name'] = "Cogcabrio", ['price'] = 70000, ['tipo'] = "carros" },
	["exemplar"] = { ['name'] = "Exemplar", ['price'] = 80000, ['tipo'] = "carros" },
	["f620"] = { ['name'] = "F620", ['price'] = 50000, ['tipo'] = "carros" },
	["felon"] = { ['name'] = "Felon", ['price'] = 80000, ['tipo'] = "carros" },
	["ingot"] = { ['name'] = "Ingot", ['price'] = 160000, ['tipo'] = "carros" },
	["jackal"] = { ['name'] = "Jackal", ['price'] = 95000, ['tipo'] = "carros" },
	["oracle"] = { ['name'] = "Oracle", ['price'] = 100000, ['tipo'] = "carros" },
	["oracle2"] = { ['name'] = "Oracle2", ['price'] = 80000, ['tipo'] = "carros" },
	["sentinel"] = { ['name'] = "Sentinel", ['price'] = 120000, ['tipo'] = "carros" },
	["sentinel2"] = { ['name'] = "Sentinel2", ['price'] = 140000, ['tipo'] = "carros" },
	["windsor"] = { ['name'] = "Windsor", ['price'] = 150000, ['tipo'] = "carros" },
	["windsor2"] = { ['name'] = "Windsor2", ['price'] = 170000, ['tipo'] = "carros" },
	["zion"] = { ['name'] = "Zion", ['price'] = 100000, ['tipo'] = "carros" },
	...
	["buccaneer2"] = { ['name'] = "Buccaneer2", ['price'] = 320000, ['tipo'] = "carros" },
	...
	["baller4"] = { ['name'] = "Baller4", ['price'] = 350000, ['tipo'] = "carros" },
	...
	["contender"] = { ['name'] = "Contender", ['price'] = 500000, ['tipo'] = "carros" },
	...
	["cognoscenti"] = { ['name'] = "Cognoscenti", ['price'] = 180000, ['tipo'] = "carros" },
	...
	["warrener"] = { ['name'] = "Warrener", ['price'] = 120000, ['tipo'] = "carros" },
	...
	["comet2"] = { ['name'] = "Comet2", ['price'] = 450000, ['tipo'] = "carros" },
	...
	["jester"] = { ['name'] = "Jester", ['price'] = 800000, ['tipo'] = "carros" },
	["khamelion"] = { ['name'] = "Khamelion", ['price'] = 210000, ['tipo'] = "carros" },
	["kuruma"] = { ['name'] = "Kuruma", ['price'] = 350000, ['tipo'] = "carros" },
	...
	["raiden"] = { ['name'] = "Raiden", ['price'] = 280000, ['tipo'] = "carros" },
	["rapidgt"] = { ['name'] = "Rapidgt", ['price'] = 200000, ['tipo'] = "carros" },
	...
	["sultan2"] = { ['name'] = "Sultan2", ['price'] = 210000, ['tipo'] = "carros" },
	...
	["entityxf"] = { ['name'] = "Entityxf", ['price'] = 500000, ['tipo'] = "carros" },
	...
	["osiris"] = { ['name'] = "Osiris", ['price'] = 670000, ['tipo'] = "carros" },
	...
	["765lt"] = { ['name'] = "Mc Laren", ['price'] = 220000, ['tipo'] = "carros" },
	...
	["guardian"] = { ['name'] = "Guardian", ['price'] = 900000, ['tipo'] = "carros" },
	["kamacho"] = { ['name'] = "Kamacho", ['price'] = 500000, ['tipo'] = "carros" },
	["neon"] = { ['name'] = "Neon", ['price'] = 450000, ['tipo'] = "carros" },
	["cyclone"] = { ['name'] = "Cyclone", ['price'] = 550000, ['tipo'] = "carros" },
	...
	["italigto"] = { ['name'] = "Italigto", ['price'] = 700000, ['tipo'] = "carros" }, 
	["schlagen"] = { ['name'] = "Schlagen", ['price'] = 500000, ['tipo'] = "carros" },
	["pgtc4"] = { ['name'] = "Porche", ['price'] = 500000, ['tipo'] = "carros" },
	["toros"] = { ['name'] = "Toros", ['price'] = 520000, ['tipo'] = "carros" }, 
	["tulip"] = { ['name'] = "Tulip", ['price'] = 150000, ['tipo'] = "carros" },
	["cb500x"] = { ['name'] = "CB 500", ['price'] = 150000, ['tipo'] = "carros" },
	["ts1"] = { ['name'] = "Ts1", ['price'] = 150000, ['tipo'] = "carros" },
	...
	["EMERUS"] = { ['name'] = "EMERUS", ['price'] = 350000, ['tipo'] = "carros" },

	--MOTOS
	
	["akuma"] = { ['name'] = "Akuma", ['price'] = 500000, ['tipo'] = "motos" },
	["avarus"] = { ['name'] = "Avarus", ['price'] = 440000, ['tipo'] = "motos" },
	["bagger"] = { ['name'] = "Bagger", ['price'] = 150000, ['tipo'] = "motos" },
	...
	["fcr"] = { ['name'] = "Fcr", ['price'] = 330000, ['tipo'] = "motos" },
	...
	["pcj"] = { ['name'] = "Pcj", ['price'] = 10000, ['tipo'] = "motos" },
	["ruffian"] = { ['name'] = "Ruffian", ['price'] = 150000, ['tipo'] = "motos" },
	...
	["shotaro"] = { ['name'] = "Shotaro", ['price'] = 700000, ['tipo'] = "motos" },
	...
	["amarokpolicia"] = { ['name'] = "AMAROK POLICIA", ['price'] = 1000, ['tipo'] = "work" },
	["ram"] = { ['name'] = "Dodge Ram 2500", ['price'] = 1000, ['tipo'] = "work" },
	["caracarastd"] = { ['name'] = "Cara Cara Policia", ['price'] = 1000, ['tipo'] = "work" },
	["carbonssd"] = { ['name'] = "Moto Policia", ['price'] = 1000, ['tipo'] = "work" },
	["coquettessd"] = { ['name'] = "Speed Policia", ['price'] = 1000, ['tipo'] = "work" },
	["komodahc"] = { ['name'] = "Komoda Policia", ['price'] = 1000, ['tipo'] = "work" },
	["sugoispd"] = { ['name'] = "Sugois Policia", ['price'] = 1000, ['tipo'] = "work" },
	...
	["rmodzl1police"] = { ['name'] = "TPD CAMARO ZL1", ['price'] = 1000, ['tipo'] = "work" },
    ["amaroknorte"] = { ['name'] = "TPD Amarok TATICAL", ['price'] = 1000, ['tipo'] = "work" },
    ["pd_i8"] = { ['name'] = "TPD I8 SEEPD", ['price'] = 1000, ['tipo'] = "work" },
    ["mercedespolicia"] = { ['name'] = "TPD Mercedes SPEED", ['price'] = 1000, ['tipo'] = "work" },
    ["pdfocus"] = { ['name'] = "TPD Focus", ['price'] = 1000, ['tipo'] = "work" },
    ["policeb"] = { ['name'] = "TPD MOTO", ['price'] = 1000, ['tipo'] = "work" },
    ["fbi2"] = { ['name'] = "VID Descaracterizada", ['price'] = 1000, ['tipo'] = "work" },
	["VRdubsta"] = { ['name'] = "Dubsta Policia", ['price'] = 1000, ['tipo'] = "work" },
	["VRamarok"] = { ['name'] = "Amarok Policia", ['price'] = 1000, ['tipo'] = "work" },
	["VRc63"] = { ['name'] = "C63 Policia", ['price'] = 1000, ['tipo'] = "work" },
	["mercxclass"] = { ['name'] = "Merceds Policia", ['price'] = 1000, ['tipo'] = "work" },
	["amg45"] = { ['name'] = "Amg45 Policia", ['price'] = 1000, ['tipo'] = "work" },
	["killjackal"] = { ['name'] = "Jackal Policia", ['price'] = 1000, ['tipo'] = "work" },
	["VRxa21"] = { ['name'] = "Xa21 Police", ['price'] = 1000, ['tipo'] = "work" },
	["VRcontender"] = { ['name'] = "Contender Policia", ['price'] = 1000, ['tipo'] = "work" },
	...
	["classexp"] = { ['name'] = "classexp", ['price'] = 1000, ['tipo'] = "work" },
...
-- NOVOS
	...
	["pcx"] = { ['name'] = "Xmax", ['price'] = 1000000, ['tipo'] = "exclusive" },
	["f800"] = { ['name'] = "Bmw F800", ['price'] = 1000000, ['tipo'] = "exclusive" },
	["gs1200"] = { ['name'] = "R1200 GS", ['price'] = 1000000, ['tipo'] = "exclusive" },
	...
	["mercedespolicia"] = { ['name'] = "Mercedes Policia", ['price'] = 1000000, ['tipo'] = "exclusive" },
