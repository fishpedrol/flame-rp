...
Dz1 = {}
Tunnel.bindInterface("dntz1_animacoes",Dz1)
vCLIENT = Tunnel.getInterface("dntz1_animacoes")
...
RegisterCommand('e2', function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id, "adm.permissao") or vRP.hasPermission(user_id, "dandan.permissao") then
	...
	if user_id then
		...
		if parseInt(#paramedicos) <= 0 then
			if vRP.tryFullPayment(user_id,5000) then
				TriggerClientEvent("Notify",source,"sucesso","Pagamento de <b>$5.000 dólares</b> efetuado e o tratamento foi iniciado.",5000)
				return true
			else
