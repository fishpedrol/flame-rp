	...
	if user_id then
		...
		if vehicle then
			...
			if placa_user_id then
				...
				local max_veh = inventory.chestweight[vname] or 20
				if sdata then
	...
	if itemName then
		...
		if user_id then
			if string.match(itemName,"dinheirosujo") or string.match(itemName,"identidade") or string.match(itemName,"mochila") then
				TriggerClientEvent("Notify",source,"importante","Sistema","Não pode guardar este item em veículos.",8000)
				return
			end
			...
			if lock == 1 then
				...
				if placa_user_id then
					...
					if not beingUsed[concat_used] then
						...
						TriggerClientEvent("Notify",source,"importante","Sistema","Você não pode utilizar este porta-malas agora.",8000)
					end
				end
			end
		end
