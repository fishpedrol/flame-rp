...
AddEventHandler('Hud:sendStatus',function(level)
	...
	SendNUIMessage({ show = IsPauseMenuActive(), vida = vida })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- FECHAR 
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- COLOCAR CINTO
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent('rusher:PoliciaToggleCinto')
AddEventHandler('rusher:PoliciaToggleCinto', function(nsource)
	if IsPedInAnyVehicle(PlayerPedId(), false) then
		if CintoSeguranca then
			...
			TriggerServerEvent('rusher:AvisarCintoPolicia', nsource, 'semcinto')
		else
			TriggerEvent("vrp_sound:source",'belt',0.5)
			TriggerServerEvent('rusher:AvisarCintoPolicia', nsource, 'comcinto')
...
-- SETAGACHADO
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent('rusher:SetAgachado')
AddEventHandler('rusher:SetAgachado', function()
	local ped = PlayerPedId()
	agachar = true
	SetPedMovementClipset(ped,"move_ped_crouched",0.30)
	SetPedStrafeClipset(ped,"move_ped_crouched_strafing")
